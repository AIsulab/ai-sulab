...
function LoginForm() {
  ...
  async function handleEmailSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        router.replace(redirectPath || "/admin/autopost");
        router.refresh();
        return;
      }
      
      // Fallback to existing placeholder if admin login fails
        ...
        <div className="w-full max-w-[420px]">
          ...
          <form onSubmit={handleEmailSubmit} className="mt-9 space-y-3.5">
            <label className="block">
              <span className="block text-[12.5px] font-medium text-slate-700 mb-1.5">이메일 또는 아이디</span>
...
export default function LoginPage() {
  return (
    <div className="min-h-screen w-full grid grid-cols-1 lg:grid-cols-2 bg-white text-slate-900">
      ...
      {/* ── RIGHT: login form ─────────────────────── */}
      <Suspense fallback={<div className="flex-1 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-violet-500" /></div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
